...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("restore-button").addEventListener("click", () => {
    if (
      confirm(
        "Are you sure you want to restore the default options? This will overwrite your current settings."
      )
    ) {
...
function setupEventHandlers() {
  ...
  document
    .getElementById("add-rss-feed")
    .addEventListener("click", () => addEntry("rss-feed-list", "RSS Feed"));

  // Add event to unmask OpenAI API key for editing
  const openaiApiKeyInput = document.getElementById("openai-api-key");
  openaiApiKeyInput.addEventListener("focus", () =>
    unmaskApiKey(openaiApiKeyInput)
  );
}
...
function saveOptions(e) {
  ...
  const ollamaEndpoint = document.getElementById("ollama-endpoint").value;
  const ollamaModel = document.getElementById("ollama-model").value;
  const googleFactCheckerEnabled = document.getElementById(
    "google-fact-checker"
  ).checked;
...
function restoreOptions() {
  browser.storage.local.get(
    ...
    (result) => {
      ...
      document.getElementById("google-fact-checker").checked =
        result.googleFactCheckerEnabled;

      // Use default RSS feeds if none are saved
      const rssFeeds =
        result.rssFeeds.length > 0
          ? result.rssFeeds
          : [
              { url: "https://www.factcheck.org/feed/", enabled: true },
              {
